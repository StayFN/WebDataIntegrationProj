 ...
 * 
 */
public class SustGoalYearNearTermFuserLongestString extends
		AttributeValueFuser<String, Company, Attribute> {

	public SustGoalYearNearTermFuserLongestString() {
	...
	public boolean hasValue(Company record, Correspondence<Attribute, Matchable> correspondence) {
		return record.hasValue(Company.SUSTGOALYEAR_NEAR_TERM);
	}

	@Override
	public String getValue(Company record, Correspondence<Attribute, Matchable> correspondence) {
		if (record.getSustGoalYear_NearTerm() == null)
			return "";
		return record.getSustGoalYear_NearTerm().toString();
	}
	...
	public void fuse(RecordGroup<Company, Attribute> group, Company fusedRecord, Processable<Correspondence<Attribute, Matchable>> schemaCorrespondences, Attribute schemaElement) {
		...
		{
			fusedRecord.setSustGoalYear_NearTerm(Integer.parseInt(fused.getValue()));
			fusedRecord.setAttributeProvenance(Company.SUSTGOALYEAR_NEAR_TERM,
